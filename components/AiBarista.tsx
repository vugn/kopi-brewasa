...
import { trackAIBaristaOpen, trackAIBaristaQuery, trackAIBaristaClose } from '../utils/analytics';
...
const AiBarista: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Track AI Barista query
    trackAIBaristaQuery(mood.length, !!result);
  ...
  return (
    <>
      ...
      {/* Modal/Panel */}
      <div
        className={`fixed bottom-6 right-6 z-50 w-80 md:w-96 bg-white rounded-2xl shadow-2xl transform transition-all duration-300 origin-bottom-right border border-brewasa-dark/10 overflow-hidden ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'
          }`}
      >
        {/* Header */}
        <div className="bg-brewasa-dark p-4 flex justify-between items-center text-brewasa-cream">
          ...
          <button
            ...
            className="hover:text-brewasa-copper transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
